...
async function fetchStreamsFromAddon(manifestUrl, type, id) {
  ...
  try {
    ...
    return streams.map(stream => ({
      ...stream,
      name: cleanStreamLabel(stream.name ? `[${addonName}] ${stream.name}` : addonName)
    }));
  } catch (e) {
...
deduped.push(s);
  ...
  return { streams: sortAndLimitStreams(deduped, 30) };
}
...
app.post("/c/create", (req, res) => {
  ...
  const { password, catalogs, mdblistKey, language, rpdbKey, tpKey, traktUser, excludeUnreleased, maxRating, streamAddons } = req.body;
