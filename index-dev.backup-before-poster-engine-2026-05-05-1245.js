...
async function tmdbResolveAiItems(items, type, language, rpdbKey, tpKey, excludeUnreleased) {
  const tmdbType = type === "series" ? "tv" : "movie";
  const found = [];
  ...
  return resultsToMetas(found, type, FILTER_ENABLED, language, rpdbKey, tpKey, excludeUnreleased);
}

async function handleCatalog(catalogId, type, extra, mdbKey, filterLang = FILTER_ENABLED, language = "en-US", rpdbKey = null, tpKey = null, traktUser = null, excludeUnreleased = false, maxRating = null, customCatalogs = [], googleAiKey = null) {
  ...
  if (catalogId === "ai_recommended_movies" || catalogId === "ai_recommended_series") {
    ...
    const metas = await tmdbResolveAiItems(items, type, language, rpdbKey, tpKey, excludeUnreleased);
    return { metas };
  }
...
deduped.push(s);
  ...
  return { streams: deduped.slice(0, 30) };
}
...
app.post("/c/create", (req, res) => {
  ...
  const { password, catalogs, mdblistKey, language, rpdbKey, tpKey, traktUser, excludeUnreleased, maxRating, streamAddons, customCatalogs, googleAiKey, enableAiRecommended } = req.body;
