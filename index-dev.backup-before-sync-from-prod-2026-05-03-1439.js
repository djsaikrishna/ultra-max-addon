...
app.get("/assets", (req, res) => {
  ...
  const cards = files.map(f => {
    ...
    return `<div style="border:1px solid #333;border-radius:12px;padding:10px;background:#111">
      <img src="${url}" style="width:100%;height:120px;object-fit:cover;border-radius:8px">
      <div style="font-size:12px;margin:8px 0;color:#ccc;word-break:break-all">${f}</div>
      <button onclick="copy('${location.origin}${url}')" style="width:100%;padding:8px;border-radius:8px;background:#18ffff22;color:#18ffff;border:1px solid #18ffff66">Copy URL</button>
    </div>`;
  }).join("");
