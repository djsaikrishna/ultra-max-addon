...
async function mdblistToMetas(listId, type, mdbKey, rpdbKey = null, tpKey = null, maxRating = null) {
...
async function handleCatalog(catalogId, type, extra, mdbKey, filterLang = FILTER_ENABLED, language = "en-US", rpdbKey = null, tpKey = null, traktUser = null, excludeUnreleased = false, maxRating = null) {
  // FORCE SEARCH NO CACHE: always bypass cached catalog rows when searching.
  ...
  // FORCE SEARCH OVERRIDE: if Nuvio sends search to any catalog, return search results.
  if(extra && extra.search){
    const q = String(extra.search).replace(/\.json$/,'').trim();
    const tmdbType = type === "series" ? "tv" : "movie";
    const language = arguments[5] || "en-US";
    ...
    console.log("CUSTOM SEARCH DIRECT:", catalogId, type, q);
  ...
  if (catalogId.startsWith("quick_")) {
    ...
    const metas = await resultsToMetas(results, type, filterLang, language, rpdbKey, tpKey, excludeUnreleased);
  ...
  if (def.handler ==="mdb") {
    ...
    return { metas: await mdblistToMetas(listId, type, mdbKey, rpdbKey, tpKey, maxRating) };
  }
...
// 🔧 Nuvio settings cog fix
// Allows /c/:token/configure to load the builder UI
app.get('/c/:token/configure', (req, res) => {
  res.sendFile(require('path').join(__dirname, 'configure.html'));
});
...
app.get("/n/:token/manifest.json", (req, res) => {
  ...
  const catalogs = buildCatalogsFromIds(config.catalogs, config.hiddenCatalogs || [])
    .map(c => ({
      type: c.type,
      id: c.id,
      name: c.name,
...
app.get("/c/:token/manifest.json", (req, res) => {
  ...
  const manifest = {
    ...
    catalogs: buildCatalogsFromIds(config.catalogs, config.hiddenCatalogs || [])
      .map(c => ({
        type: c.type,
        id: c.id,
        name: c.name,
...
console.log("CUSTOM CATALOG:", token, id, "extraStr:", extraStr);
      ...
      if (extraStr) {
        try {
          extra = JSON.parse(decodeURIComponent(extraStr));
        } catch {
          decodeURIComponent(extraStr).split('&').forEach(part => {
      ...
      // Nuvio/Stremio search extra can arrive as /search=query.json
      if(extra.search){
        extra.search = String(extra.search).replace(/\.json$/,'').trim();
      }
      // Direct search handling for Nuvio/Stremio search extras.
      // Some catalog handlers ignore extra.search, so catch it here first.
      if(extra.search){
        const q = String(extra.search).replace(/\.json$/,'').trim();
        const tmdbType = type === "series" ? "tv" : "movie";
        const language = config.language || "en-US";

        console.log("CUSTOM SEARCH DIRECT:", token, type, id, q);

        if(q){
          fetchCached(`https://api.themoviedb.org/3/search/${tmdbType}?api_key=${TMDB_KEY}&query=${encodeURIComponent(q)}&page=1&language=${language}`)
            .then(data => resultsToMetas(data.results || [], type, false, language, config.rpdbKey || null, config.tpKey || null))
            .then(metas => {
              res.setHeader("Cache-Control","public, max-age=60");
              res.json({ metas });
            })
            .catch(err => {
              console.log("CUSTOM SEARCH ERROR", err.message);
              res.json({ metas: [] });
            });
          return;
        }
